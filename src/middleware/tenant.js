...
const tenantMiddleware = async (req, res, next) => {
    ...
    try {
      ...
      if (users.length === 1) {
        ...
        return next(
          new ApiError(
            400,
            "Multiple tenants found for this email. Please provide the subdomain."
          )
        );
      }
    } catch (error) {
      return next(
        new ApiError(500, "Error identifying tenant from email", [
          error.message,
        ])
      );
    }
  ...
  if (!subdomain) {
    return next(
      new ApiError(
        400,
        "Tenant subdomain is required (header: x-tenant-subdomain or body: subdomain)"
      )
    );
  }
  ...
  try {
    const tenant = await Tenant.findOne({
      where: { subdomain: req.subdomain, is_active: true },
    });
