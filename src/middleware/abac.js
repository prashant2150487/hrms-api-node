...
export const abac = (permission, options = {}) => {
  return async (req, res, next) => {
    try {
      ...
      if (options.checkOwnership || options.checkManager || options.checkDepartment) {
        ...
        if (!options.model) {
          return next(new ApiError(500, "ABAC Configuration error: Model is required for attribute checks"));
        }
        ...
        const userEmployee = await Employee.findOne({ where: { user_id: user.id } });
        ...
        if (userEmployee) {
          ...
          if (options.checkManager) {
            ...
            if (options.model.name === 'Employee' && resource.manager_id === userEmployee.id) {
              return next();
            }
            // If resource has employee_id, check that employee's manager
            if (resource.employee_id) {
              const resourceEmployee = await Employee.findByPk(resource.employee_id);
              if (resourceEmployee && resourceEmployee.manager_id === userEmployee.id) {
                return next();
              }
            }
          }
