...
export const createLeavePolicy = async (req, res) => {
  try {
    ...
    const { leave_type, days_allowed, carryover_limit, applicable_to } =
      req.body;
...
export const updateLeavePolicy = async (req, res) => {
  try {
    const { tenant_id } = req.user;
    const { id } = req.params;
    const { days_allowed, carryover_limit, applicable_to, is_active } =
      req.body;
  ...
  try {
    ...
    if (existingLeaveRequest) {
      return res.status(409).json({
        success: false,
        message:
          "Leave request already exists for the same employee, leave type, and date range",
      });
    }
    ...
    leaveBalance.balance =
      Number(leaveBalance.balance) - Number(number_of_days);
