...
const fixUserTable = async () => {
  try {
    ...
    await sequelize.query(
      'ALTER TABLE "users" ALTER COLUMN "is_active" TYPE BOOLEAN USING (is_active::integer::boolean);'
    );
    ...
    await sequelize.query(
      'ALTER TABLE "users" ALTER COLUMN "is_email_varified" DROP DEFAULT;'
    );
    await sequelize.query(
      'ALTER TABLE "users" ALTER COLUMN "is_email_varified" TYPE BOOLEAN USING (is_email_varified::integer::boolean);'
    );
    await sequelize.query(
      'ALTER TABLE "users" ALTER COLUMN "is_email_varified" SET DEFAULT false;'
    );
    console.log("Changed is_email_varified type to BOOLEAN");

    // 4. Fix mfa_enabled if it was changed
    await sequelize.query(
      'ALTER TABLE "users" ALTER COLUMN "mfa_enabled" DROP DEFAULT;'
    );
    await sequelize.query(
      'ALTER TABLE "users" ALTER COLUMN "mfa_enabled" TYPE BOOLEAN USING (mfa_enabled::integer::boolean);'
    );
    await sequelize.query(
